<%--
  Created by IntelliJ IDEA.
  User: Pc
  Date: 12/30/2022
  Time: 12:40 PM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>

<script src=" <c:url value="/templates/commons/js/bootstrap.js" /> "></script>
<script type="text/javascript" src="http://maps.google.com/maps/api/js?sensor=true"></script>
<script type="text/javascript" src=" <c:url value="/templates/web/js/gmaps.js" /> "></script>
<script src=" <c:url value="/templates/web/js/contact.js" /> "></script>
<script src=" <c:url value="/templates/web/js/price-range.js" /> "></script>
<script src=" <c:url value="/templates/web/js/jquery.scrollUp.min.js" /> "></script>
<script src=" <c:url value="/templates/web/js/jquery.prettyPhoto.js" /> "></script>
<script src=" <c:url value="/templates/web/js/main.js" /> "></script>